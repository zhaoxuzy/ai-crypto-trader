...
3. 如果指出的错误是致命的（数据造假、逻辑断裂、方向与第一段运动矛盾），你应该果断推翻原方案。
4. 如果指出的错误是非致命的（止损过紧、仓位偏重、某个步骤推理略弱），你应该修正参数而非推翻方向。
5. 如果质疑本身站不住脚，而原推演逻辑坚实，你应该驳回质疑，维持原方向。
6. 如果你认为争议点处于不可判定的灰色地带，你可以主动降仓、收紧止损，输出一份更保守的方案。
7. 如果你无法输出更稳健的方案，你可以输出观望。但观望必须是最后的选择，而不是默认选项。
