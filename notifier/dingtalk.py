import os, time, hmac, hashlib, base64, urllib.parse, requests, re
...
def format_reasoning(text: str) -> str:
    if not text: return "> 无推理过程"
    text = text.replace('\r\n', '\n').replace('\r', '\n')
    text = re.sub(r'\n{3,}', '\n\n', text)
    titles = ["第[一二三四五六]步[：:]","分析数据[：:]","第一反应[：:]","自我质疑[：:]","最终结论[：:]",
              "交叉验证与裁决[：:]","价格路径推演[：:]","推理自检[：:]","入场区间[：:]","止损位[：:]","止盈位[：:]"]
    for title in titles:
        text = re.sub(rf'(?<!\n)({title})', r'\n\1', text)
    lines = text.split('\n')
    quoted = []
...
def format_strategy_message(symbol: str, strategy: dict, data: dict) -> str:
    ...
    if direction == "neutral":
        ...
        if size_cn: parts.append(size_cn)
        parts.append(conf_cn); parts.append(now)
        title = "## " + " · ".join(parts)
    ...
    if not risk_lines: risk_lines = ["> 请严格设置止损"]
    risk_block = "> ### ⚠️ 风险说明\n" + "\n".join(risk_lines)
