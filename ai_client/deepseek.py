...
最终合约策略：基于前六步的综合分析，我决定【long/short/neutral】，置信度设置为 high / medium / low，仓位设计为 heavy / medium / light，入场区间+止损位+止盈位，说明具体的依据、理由或核心逻辑。
主动证伪信号：
微观盘口确认：
...
【审计报告】{report}
【完整市场数据（所有字段必须以引用的方式出现）】{market_data_str}
...
【输出模板】
 必须按照以下模板输出最终策略（输出格式要求：纯文本，不要输出JSON或其他任何格式），否则视为无效输出。
