...
deepseek.py — 优化七步分析法
核心改进：
1. 第一步增加 [趋势阶段判断]，引导关注中期指标
2. 第七步增加 [方向自检清单]，强制自我审查与锚点服从
3. 铁律强化：趋势优先、胜率优先、锚点冲突强制观望
"""
...
【铁律】
0. **趋势优先**：优先参考LTH/STH成本、稳定币趋势、BTC占比趋势等中期指标。分钟级净流、CVD等仅用于验证，不作为方向判断的主要依据。
0. **胜率优先**：在没有60%以上把握时，应首选“观望”。环境评分<40分时，禁止开仓。
1. 只能使用下方市场数据，严禁任何外部知识或记忆数值。
2. 每一步必须按 [分析数据] → [第一反应] → [自我质疑] → [最终结论] 结构推进，缺一不可。
3. [分析数据] 中必须以填空方式直引表格中的对应数值，不可省略任何一项。
4. 最终方向必须与 direction_bias 同向。若相反，必须输出“观望”并说明理由。当 |direction_bias| > 0.4 且方向相反时，禁止任何形式的反驳，直接输出“观望”。
...
[趋势阶段判断]（必须完成）
- 当前价格相对于LTH成本（{lth_rp_str}）和STH成本（{sth_rp_str}）的位置：___
- STH SOPR（{sth_sopr_str}）显示短期持有者处于[盈利/亏损/平衡]
- 稳定币市值趋势（{stable_trend_str}%）显示资金[流入/流出/持平]
- 综合判断：市场处于[积累/上升/派发/下跌]阶段
[最终结论]
环境评分（0-100）：__分
...
第七步：制定交易计划
[综合前六步结论，并结合系统客观锚点 **direction_bias = {direction_bias:.3f}** 给出最终策略。]

【方向自检清单】（必须逐项回答，否则策略无效）
1. 我推演得出的方向是：[做多/做空/观望]
2. 支撑该方向的最强三个数据字段及当前数值：
   a. （字段名：数值 → 解读）
   b. 
   c. 
3. 是否存在与该方向矛盾的数据？如果有，是什么？（必须至少写一个，若确实没有则写“无”）
4. 系统锚点 direction_bias = {direction_bias:.3f}，指向[偏多/偏空]。
   - 我的方向与锚点：[一致/相反]
   - 若相反，我必须输出“观望”，理由：___
5. 第一步环境评分为___分。
   - 若 < 40 分，我必须输出“观望”，理由：___
...
【强制输出要求】
请将以上七步完整推演及【方向自检清单】全部写入 "reasoning" 字段，不可省略任何一步。若 "reasoning" 为空或内容不完整，该策略将被系统自动驳回。
输出JSON（不要加代码块标记）：
{{
  "direction": "做多 / 做空 / 观望",
  "confidence": "高 / 中 / 低",
  ...
  "reasoning": "务必包含第一步到第七步的完整推演及方向自检清单，每步均需有数值和分析、第一反应、自我质疑、最终结论，不得空缺。",
  "risk_note": "核心风险（20字内）",
