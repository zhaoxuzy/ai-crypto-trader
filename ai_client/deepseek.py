...
def build_judge_prompt(original_strategy: dict, reviewer_report: dict, data: dict, symbol: str) -> str:
    ...
    prompt = f"""你是交易委员会主席，拥有二十年加密货币短线合约实战经验，经历过多次极端行情与黑天鹅事件。你的团队已为你提交了两份材料：1. 【首席交易员的策略】2. 【风控审计报告】，你必须基于这两份材料中的事实与逻辑，结合自己的实战经验输出一份可立即执行的短线加密货币交易方案。
...
【裁决流程】
你必须严格遵循以下三个步骤进行，不可跳过或自由发挥。
STEP 1 – 审计逐条裁决
对审计报告中的每一条指控，你必须进行客观裁决：
  - 裁决结论（采纳 / 驳回 / 部分采纳）
  - 核验依据（必须引用上方“市场数据”中的字段名和具体数值或专业的判断）
  - 反证风险评估（高/中/低）。你必须提出至少一条可能挑战你裁决的证据。反证必须包含：该证据是什么、它为何可能构成威胁、以及你为何最终排除它。若你搜寻了所有数据后确实认为毫无矛盾，才可填写“无”，但必须简述你的搜寻范围作为理由。

STEP 2 – 制定最终策略
  (A) 判断你是否维持原策略的方向。
  (B) 如果你决定**推翻原方向**，你必须同时完成以下四项工作，缺一不可：
      1) 明确指出原方向所依赖的核心假设（一句话概括即可，例如：“假设市场将因卖盘衰竭而反弹”）。
