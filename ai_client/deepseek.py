...
第七步：最终交易计划
你必须综合前六步结论，并结合系统客观锚点 **direction_bias = {data['direction_bias']:.3f}** 给出最终策略。最终方向必须与 direction_bias 符号一致。若不一致，必须在下方逐条列出至少三项基于具体数据字段的反驳理由，并自动将置信度降为 'low'。若输出 neutral，所有价格字段为0，仓位为none。
...
请逐层检查遗漏，输出：步骤X 遗漏[指标名]（数值）。若全覆盖写“无”。[严重性：高/中/低]]

二、数据/解读错误
核验交易员引用的数值是否与市场数据一致；解读方向是否正确。若有误，输出：步骤X 错误（原文/正确值）。无则写“无”。[严重性：高/中/低]]

三、逻辑矛盾
检查推演是否自相矛盾（如痛苦度与猎物方向冲突、资金流背离未降级）。若有，输出具体矛盾。无则写“无”。[严重性：高/中/低]]

四、关键反证与锚点对齐
1. 若|direction_bias|>0.4且交易员反向且未列三项反驳 → 严重性高，建议驳回。
2. 若交易员未引用direction_bias → 缺失。
3. 交易员结论与市场数据中是否存在未处理的重大反向信号？请列举。[严重性：高/中/低]]
