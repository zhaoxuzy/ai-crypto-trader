"""
deepseek.py — 七步分析法优化版
核心改进：
1. 第七步嵌入「方向决策汇总」：三组独立指标投票决定最终方向。
2. 铁律强化：方向矛盾强制观望、锚点冲突强制观望、盈亏比硬约束 ≥ 2:1。
3. 第一步增加趋势阶段判断，指引长线背景。
"""
...
def validate_strategy(s: dict, data: dict = None) -> tuple[bool, str]:
    ...
    if direction == "long" and stop_loss >= entry_low:
        s["risk_note"] = s.get("risk_note", "") + " ⚠️止损位需在入场区间下方。"
    elif direction == "short" and stop_loss <= entry_high:
        s["risk_note"] = s.get("risk_note", "") + " ⚠️止损位需在入场区间上方。"
...
"""
    ...
    # ========== 拼接完整 prompt ==========
    prompt = f"""你是加密货币交易团队的首席交易员，负责分析市场数据并制定交易计划草案。你的核心任务是：基于客观数据，通过三组独立的方向信号投票，得出最可靠的交易方向。

【铁律】
0. **方向决策优先**：方向必须由第七步开头的「方向决策汇总」投票决定，前六步为投票提供分析依据。
1. **三组投票制**：资金流、持仓情绪、链上盈亏三组各自独立判断方向，最终方向由投票结果决定。若两组以上矛盾，必须输出"观望"。
2. **锚点服从**：若投票得出的方向与系统锚点 direction_bias 不一致，必须降级仓位或直接观望。当 |direction_bias| ≥ 0.4 且方向相反时，禁止任何形式的反驳，直接输出"观望"。
3. **盈亏比硬约束**：非观望信号必须满足 (止盈-入场) / (入场-止损) ≥ 2:1，否则降为观望。
4. 只能使用下方市场数据，严禁任何外部知识或记忆数值。
5. 每一步必须按 [分析数据] → [第一反应] → [自我质疑] → [最终结论] 结构推进，缺一不可。
6. [分析数据] 中必须以填空方式直引表格中的对应数值，不可省略任何一项。
7. 所有步骤（第一到第七步）必须完整写入 "reasoning" 字段，不可为空。
...
（正值=偏多，负值=偏空，绝对值>0.4为强方向信号）

--------------------------------
第一步：环境定调
[分析数据] 直引填空：价格7日分位__%，1h波动率__%，波动因子__，CGDI当前__（分位__%），恐慌贪婪__（7日前__）。合约基差__（分位__%），稳定币市值趋势__%，借贷利率__%。LTH成本__，STH成本__，STH SOPR__。
[第一反应] 当前市场温度与稳定性如何？是否适合狩猎？
[趋势阶段判断]（必须完成）
- 当前价格相对于LTH成本和STH成本的位置：___
- STH SOPR显示短期持有者处于[盈利/亏损/平衡]
- 稳定币市值趋势显示资金[流入/流出/持平]
- 综合判断：市场处于[价值区/拉升区/派发区/下跌区]
[自我质疑]
- CGDI与恐慌贪婪是否同时指向极值？基差是否异常？
- 波动因子是否>2.0？稳定币市值是扩张还是萎缩？
[最终结论]
环境评分（0-100）：__分
仓位硬上限：≥70分 → 可重仓；40-70 → 中/轻仓；<40 → 无仓位，必须观望。
...
4. 当前价格是否已触及LTH成本线？这对长期持有者意味着什么？
[最终结论]
...
[自我质疑]
1. 计算有效引爆距：上方触发距 + 卖方大单缓冲 ≈ __；下方触发距 + 买方大单缓冲 ≈ __。
2. 诱饵检测：若清算密集侧与大单压迫方向相同，则可能是诱饵陷阱。
3. 对立池威胁评估：对立池清算比值>1.5意味着反向威胁高。
4. 结合第一步的[趋势阶段]，当前地形是确认趋势方向，还是制造陷阱？
[最终结论]
第一段最可能运动方向：向上 / 向下
...
第七步：制定交易计划
[综合前六步分析，完成下述方向决策汇总，并基于投票结果制定策略。]

【方向决策汇总】（必须逐项完成，不可跳过）

第一组：资金流方向（三选一投票，多数决定）
  - CVD斜率：{cvd_slope_str} → 判断：[多/空/中性]
  - 主动买卖比(1h)：{taker_str} → 判断：[多/空/中性]
  - 24h期货净流：{nf24h_str}M → 判断：[多/空/中性]
  本组投票结果：__/3 票 → 资金流方向：[看多/看空/中性]

第二组：持仓与情绪方向（三选一投票，多数决定）
  - OI 24h变化：{oi_chg_str}% → 判断：[多/空/中性]
  - 资金费率分位：{fund_perc_str}% → 判断：[多/空/中性]
  - 恐慌贪婪指数：{fear_greed} → 判断：[多/空/中性]
  （注：恐慌贪婪<25或>75时权重翻倍，视为2票）
  本组投票结果：__/3票 → 情绪方向：[看多/看空/中性]

第三组：链上盈亏方向（单指标 + 辅助验证）
  - STH SOPR：{sth_sopr_str} → 判断：[多/空/中性]
  - 现价 vs STH成本：{mark_price_str} vs {sth_rp_str} → [高于/低于] → 信号：[强化/弱化]
  本组结论：[看多/看空/中性]
...
【三组合成】
  ...
  情绪：  [看多/看空/中性]
  链上：  [看多/看空/中性]
  最终方向投票：__/3 组一致 → 结论：[做多/做空/观望]

  与锚点比对：direction_bias = {direction_bias:.3f} → [一致/冲突]
  → 最终仓位修正：[重仓/中仓/轻仓/无]
...
- 置信度：[高/中/低]
- 入场区间：[__-__] (依据：__)
- 止损：[__] (依据：设置于关键支撑/阻力外+1-2倍ATR缓冲。{atr_str}为4h ATR参考 )
- 止盈：[__] (依据：清算密集区或期权痛点)
- 盈亏比计算：(止盈-入场)/(入场-止损) = __/__ = __:1 [必须≥2:1]
- 说明：[一句话指令或观望触发条件]

【强制输出要求】
请将以上七步完整推演及【方向决策汇总】全部写入 "reasoning" 字段。
输出JSON（不要加代码块标记）：
{{
  "direction": "做多 / 做空 / 观望",
  "confidence": "高 / 中 / 低",
  ...
  "reasoning": "务必包含第一步到第七步的完整推演及方向决策汇总，每步均需有数值和分析、第一反应、自我质疑、最终结论，不得空缺。",
  "risk_note": "核心风险（20字内）",
  "final_strategy": "请将第七步最终合约策略填入此字段。"
}}
"""
    return prompt
