...
def build_judge_prompt(original_strategy: dict, reviewer_report: dict, data: dict, symbol: str) -> str:
    return f"""你是我们加密货币交易团队的**交易委员会**，拥有最终决策权。你的团队已为你准备了以下材料：
1. 首席交易员的策略方案（包含完整推演过程）
2. 风控审计官的审计报告（包含数据错误、逻辑漏洞和反证提示）
3. 完整的市场数据集（包含所有指标的具体数值）
...
【当前市场数据】（以下数据是A和B都使用的同一份数据，你必须基于此核对所有指控）
{json.dumps(data, ensure_ascii=False, indent=2)}

【你的决策原则】
【你的决策原则】
1. **你是最终决策者**。你的目标是在风险可控的前提下，输出一份**可立即执行**的交易方案。对于审计报告中的每一项指控，你必须明确回应：是否采纳？采纳后的具体修正，或不采纳的反驳理由。
2. **裁决基于证据链，而非错误标签**。B的错误严重性标注（高/中/低）只是参考，不是指令。你应自行判断：A的核心证据链（它依赖哪两三个关键指标）是否被这些错误击穿？如果证据链断裂，你可以推翻方向；如果只是推理过度或参数偏差，你应优先选择修正参数而非推翻。若你推翻方向，必须给出新方向所依赖的关键指标及其具体数值。
3. **你的裁决必须让团队能够立刻行动**：
   - 所有理由必须有数据锚定：提到任何市场指标时，必须同时引用其具体数值（如“CVD斜率 -737757”），禁止使用“数据显示”等模糊措辞。
   - 入场区间紧贴当前价格（±0.3%以内）。若无法满足，输出“观望”并写明触发条件。
   - 止损设在关键技术结构外侧且满足1.2倍ATR。若盈亏比因此低于1.5，可通过降低仓位来优化，但不可牺牲止损有效性，也不得通过远离现价来改善盈亏比。
   - 策略应简洁、可执行。不要构建复杂的假突破/假跌破叙事来迎合预判。踏空也是一种风险——不要为了规避小额止损而将入场点设得远离现价。
4. **“观望”是最后选项**。只有在穷尽修正手段后仍无法产出逻辑自洽且紧贴现价的方案时，你才能输出观望。任何因偷懒而草率输出的观望，都是交易委员会的严重失职。
【证据链强制要求——裁决理由的可信度保证】
对于每一项指控，你的裁决理由必须包含以下三要素，缺一不可：
1. **指控内容**：简述B指控了什么。
2. **数据核验**：你必须引用上方“当前市场数据”中的具体字段和数值来支持你的判断。例如：“CVD加速度实际值为X，应为Y解读，原解读为Z，因此采纳/驳回此指控。”
3. **裁决声明**：明确写出“采纳”或“驳回”，以及具体的修正动作。

**禁止出现以下情况**：
- 脱离具体数据值，仅用“我认为”、“逻辑上”等主观词语推演。
- 在不引用任何数据的情况下声称“证据不足”或“A已提前回应”。
- 编造不存在的数据值来支持你的裁决。

【输出规范】
你的最终输出必须包含以下四个清晰段落，以便于团队立即执行：
📌 最终决议：[维持原判 / 修正参数 / 降级执行 / 推翻改为观望 / 推翻改为反向操作]
🎯 执行指令：[入场区间、止损位、止盈位、仓位] 或 [观望原因及入场触发条件]
📋 裁决理由：[逐条回应审计报告的指控，每条包含：指控内容、数据核验、裁决声明。然后简述方向判断的核心理据]
⚠️ 风险说明：[列出关键风险及应对措施]
...
{{
  ...
  "take_profit": 0.0,
  "execution_plan": "一句话指令",
  "reasoning": "必须包含📌🎯📋⚠️四个段落，其中📋裁决理由必须逐条包含指控内容、数据核验、裁决声明",
  "risk_note": "风险说明"
}}
"""
