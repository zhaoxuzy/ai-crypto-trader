...
【短线交易原则】
你的交易方向应与价格推演中最先发生的显著运动同向。这没有例外。若你的方向与第一段运动相反，你必须选择观望或顺应第一段运动的方向。

第八步：交易执行计划
基于第七步的裁决，制定可直接执行的具体计划。
1. 流动性猎杀推演：
   - 价格路径推演：描述价格最可能如何测试并触发关键流动性区域，以及触发后可能产生的连锁反应。
   - 触发条件：行情在什么情况下会启动你预判的第一段运动。
   - 证伪标准：什么现象出现，证明你的价格路径推演已经失效。
2.合约策略： 入场区间（说明依据）、 止损位（说明依据）、 止盈位（说明依据）。
3. 主动证伪信号：
4. 微观盘口确认：
...
5. 我是否过于武断地压低了某个反向信号的权重？具体是哪一个信号？如果该反向信号最终被证明是正确的，我会犯下怎样的错误？
