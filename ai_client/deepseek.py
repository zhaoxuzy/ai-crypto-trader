...
【强制输出要求】
请将以上七步完整推演全部写入 "reasoning" 字段，不可省略任何一步，不可用“略”替代。若 "reasoning" 为空或内容不完整，该策略将被系统自动驳回。
输出JSON（不要加代码块标记）：
{{
  "direction": "做多 / 做空 / 观望",
  "confidence": "高 / 中 / 低",
        ...
        try:
            ...
            if exec_block:
                ...
                if dir_match:
                    ...
                    if "做多" in first_200:
                        direction = "long"
                    elif "做空" in first_200:
                        direction = "short"
                    elif "观望" in first_200:
                        direction = "neutral"
