...
最终合约策略：基于前六步的综合分析，我决定输出【long/short/neutral】，置信度设置为 high / medium / low，仓位设计为 heavy / medium / light，入场区间+止损位+止盈位，说明具体的依据、理由或核心逻辑。
主动证伪信号：
微观盘口确认：
...
{{
  ...
  "reasoning": "第一步到第七步的完整推演文本，包含所有分析、第一反应、自我质疑、最终结论、价格路径推演、最终合约策略",
  "risk_note": "风险说明"
}}
"""
    return prompt
...
def build_reviewer_prompt(original_strategy: dict, data: dict, symbol: str) -> str:
    ...
    return f"""你是加密货币交易团队的风控审计官，唯一任务是在首席交易员提交的策略中找出所有错误。你的审查标准：快速、详尽、只找错误，不写裁决、不写建议、不写正确做法。
...
【风控审计官 - 审计报告】
一、遗漏指标与分析缺失：
1.[若有遗漏，按此格式：在[步骤X/决策点]中，交易员未分析已提供的[指标名称/数据项]。该指标显示[具体数值/信号]，若纳入分析将[强化/削弱/推翻]当前方向判断。 [严重性：高/中/低]]
[若无遗漏，写“已覆盖所有应分析的关键指标”]
2.
二、数据与解读错误：
1.[若有错误，按此格式：在[步骤X]中，交易员声称[数值/解读]，但实际数据为[数值/正确含义]。此错误[是否影响方向判断]。 [严重性：高/中/低]]
[若无，写“未发现数据或解读错误”]
2.
三、逻辑错误：
1.[若有错误，按此格式：[错误类型]在[步骤X]：[描述]。 [严重性：高/中/低]]
[若无，写“未发现明显逻辑错误”]
2.
四、关键反证提示：
1.[若原策略推演中引用的数据或逻辑，与已提供的其他数据存在明显矛盾，按此格式指出：在[步骤X]中，策略依据[数据A]得出[结论]，但已提供的[数据B]显示[相反信号]，二者构成矛盾，未被交易员处理。 [严重性：高/中/低]]
[若无矛盾，写“未发现关键反证被忽略”]
2.
五、博弈层面审视：
1.基于已提供的清算池评分，做市商更可能向哪个方向猎杀流动性？策略的止损位是否暴露在该路径上？（若数据不足以判断，写明“数据不足，无法判定”）
2.预设的入场区间，是否与已提供图表中的关键结构位（前高/前低/成交密集区）重合，从而可能成为对手盘的流动性来源？（若数据不足以判断，写明“数据不足，无法判定”）
"""
...
【强制输出纪律-违反视为无效输出】
- 你输出的“📋 裁决说明”必须逐一回应审计报告中的【所有指控】，不得合并、不得省略、不得用“同上”代替。
- 每一条指控必须严格按照以下格式输出，缺一不可：
    1. 指控内容：[原文概括]
       裁决结论：采纳/驳回/部分采纳
       核验依据：（引用字段+数值）
       反证风险评估：（高/中/低，证据及排除理由）
- 若审计报告有 N 条指控，你的输出就必须有 N 条裁决。遗漏任何一条将被视为无效裁决。

【输出模板】
 必须按照以下模板输出最终策略（输出格式要求：纯文本，不要输出JSON或其他任何格式），裁决说明必须完整，否则视为无效输出。
...
【输出自检 - 请务必在生成后确认】
- 我是否已逐条列出审计报告中的全部指控？（是/否）
- 每条是否都包含“核验依据”和“反证风险评估”？（是/否）
