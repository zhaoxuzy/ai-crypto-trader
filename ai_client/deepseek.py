...
第七步：矛盾裁决与最终决策
请以顶尖交易员的身份，独立完成以下裁决流程：
1. 汇总并标注：逐条列出前六步得出的所有最终结论，明确标注每条结论的方向属性（看涨/看跌/中性）。
2. 独立赋权并说明依据：为每一条结论赋予一个你认为合理的权重（所有权重之和为100%）。必须清晰写出你如此分配权重的逻辑——结合当前市场环境，解释为什么某些证据被赋予更高权重，某些被刻意压低。
3. 强制回答三个裁决问题：
   a) 最不可忽视的信号是什么？在当前市况下，哪一个信号你绝不敢忽视？它为何具备如此决定性的分量？
   b) 最强的反向信号是什么？与你的最终方向相反的最强证据是哪一个？你究竟是选择压过它，还是被它说服？请明确写出你作出这种取舍的核心依据。
   c) 核心假设与证伪条件是什么？你的最终方向，是建立在哪一两个核心假设之上的？请清晰列出假设，并定义“证伪条件”——即一旦盘中出现什么具体现象或数据，就说明假设已被推翻，你必须立即改变方向。
4. 输出最终裁决：给出明确的结果方向：看涨 / 看跌 / 观望，并附上对应的核心假设与证伪条件。
随后，请以顶尖交易员视角专业进行价格路径推演（流动性猎杀推演）：基于当前清算池分布、对手盘结构和资金流方向，描述价格最可能如何测试并触发关键流动性区域，以及触发后可能产生的连锁反应。需包含触发条件和证伪标准。

推理自检：
1. 我的最终裁决是否完全基于前六步的数据和结论？有无遗漏，指标解读是否专业准确？
