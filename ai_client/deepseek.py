...
第七步：信号计分与仓位决策
你必须基于前六步的“最终结论”，给出以下6个维度的子信号，每个维度+1（偏多）、0（中性）、-1（偏空）：
| 维度 | 信号 | 依据（一句话） |
|------|------|----------------|
| 环境 (来自第一步) | [+1/0/-1] | 波动率/分位数状态… |
| 猎物方向 (来自第二步) | [+1/0/-1] | 最小阻力路径… |
| 对手盘极端度 (来自第三步) | [+1/0/-1] | OI+资金费率极值含义… |
| 资金流 (来自第四步) | [+1/0/-1] | CVD/净流方向… |
| 辅助信号 (来自第五步) | [+1/0/-1] | 期权痛点/汇率… |
| 跨币种共振 (来自第六步) | [+1/0/-1] | 方向一致则取同向，背离取0… |
计分规则：
- 总分 = 各维度信号之和（范围 -6 到 +6）
- 信号一致性 = 同向维度数 / 总有效维度数（忽略0分维度）
- 矛盾度 = 反向维度是否存在且绝对值大于1（即出现 -1和+1同时存在）
仓位与置信度映射：
- 总分 ≥4 且 信号一致性 ≥0.8 且 无强矛盾 → 方向跟随总符号，仓位 [重仓]，置信度 [高]
- 总分 ≥2 且 信号一致性 ≥0.65 且 无强矛盾 → 方向跟随总符号，仓位 [中仓]，置信度 [中]
- 总分 ≥1 且 一致性 ≥0.5 → 方向跟随总符号，仓位 [轻仓]，置信度 [低]
- 其他情况 → 观望，仓位 [无]，置信度 [低]
（若总分在关键阈值边缘且有一项矛盾度偏高，强制降一级仓位；若总分虽高但跨币种共振为0，降一级置信度）
...
{{
  ...
  "reasoning": "第一步到第八步的完整的最终结论以及最终合约策略",
  "risk_note": "风险说明"
}}
"""
    return prompt
